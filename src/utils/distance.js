  ...
  useEffect(() => {
    ...
    async function setupLocation() {
      try {
        ...
        subscription = await watchUserLocation((coords) => {
          setUserGps(coords);
        });
        ...
        <SafeAreaView style={s.overlaySafe} pointerEvents="box-none">
          ...
          <View style={s.stageWrap}>
            <View style={s.stageCard}>
              <Text style={s.stageLabel}>Mode</Text>
              <Text style={s.stageValue}>{stage.label}</Text>
...
const s = StyleSheet.create({
  ...
  stageCard: {
    backgroundColor: PSU.stageBg,
    borderRadius: 18,
    borderWidth: 1,
    borderColor: PSU.stageBorder,
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  stageLabel: {
